...
def create_gm_acc_spam_command(packer, controller, CS, slc_set, bus, Vego, accel, experimentalMode, sdgm, frogpilot_toggles):
  cruiseBtn = CruiseButtons.INIT
  byfive = 0
  ...
  if not experimentalMode:
    if slc_set + 5 < Vego * MS_CONVERT:
      slc_set = slc_set - 10 # 10 lower to increase deceleration until with 5
  else:
    slc_set = int(round((Vego * 1.01 + 4.6 * accel + 0.7 * accel ** 3 - 1 / 35 * accel ** 5) * MS_CONVERT)) # 1.01 factor to match cluster speed better
